...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let bot;
    top = x1*x2+y1*y2;
    bot = Math.abs(Math.sqrt(x1*x1+y1*y1))*Math.abs(Math.sqrt(x2*x2+y2*y2));
    let res = top/bot;
...
function roundToPowerOfTen(num, pow) {
    ...
    let ten = (1+''+'0'.repeat(pow))*1;
    if (pow == 0) 
        return num;
    else 
        return Math.round(num/ten)+'0'.repeat(pow);
}
...
function toNumber(value, def) {
    if (value*1 == Number(value)*1) return Number(value);
    else return def;
}
